...
def url_label_concat(res, url_file_path="../data/url_list_" + str(datetime.date.today().strftime("%Y%m%d")) + ".csv"):
    ...
    for line in res[1:]:
        ...
        if int(line[2]) == 0:
            line_new.append(1.0 - float(line[-1][-1]))
        else:
    ...
    out_put = out_put[["productinfo_id", "image_id", "image_url", "image_format", "create_time", "proba", "label"]]
    return out_put
...
if __name__ == '__main__':
    ...
    out_put = url_label_concat(res, url_file_path)
    print(out_put)
